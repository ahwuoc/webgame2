...
$.fn.customSelect = function (options) {
	...
	const outer = $(this);
	const inner = outer.find("select");
	const otps = inner.find("option");
	const selectContentEl = $(`<div class="select-display">${inner.val()}</div>`);
	outer.append(selectContentEl);

	// add select item
	const items = [];
	const itemListEl = $(`<div class="select-item-list"></div>`);
	otps.each(function (index, el) {
		const div = $(`<div class="select-item">${el.value}</div>`);
		itemListEl.append(div);
		items.push(div);
	});
	outer.append(itemListEl);
	...
	items.forEach(function (item, index) {
		item.on("click", function (e) {
			...
			selectContentEl.html(item.html());
			inner.val(otps[index].value);
		});
	});
...
var config = {
	...
	func: {
		isMobile: function () {
			...
			return device.width < 999 && device.width < device.height;
		},
	},
};
...
$.fn.scalePlatform = function (options) {
	...
	var isMobile = (width = device.width, height = device.height) => width < 999 && width <= height;
	...
	$(settings.elScale.desktop + " , " + settings.elScale.mobile).each(function () {
      ...
      if (document.querySelector(".layer")) {
				...
				setTimeout(function () {
					if (document.querySelector(".layer")) document.querySelector(".layer").remove();
				}, 400);
			}
		}
	});
	...
	if (details.length > 0) {
		details.css({
			height: `${details.find(".news-bg").outerHeight(true)}px`,
		});
	}
	...
	if (faqs.length > 0) {
		faqs.css({
			height: `${faqs.find(".faq-container").outerHeight(true)}px`,
		});
	}
...
// UNUSED EXPORTS: A11y, Autoplay, Controller, EffectCards, EffectCreative, EffectCube, EffectFade, EffectFlip, FreeMode, Grid, HashNavigation, History, Keyboard, Lazy, Manipulation, Mousewheel, Parallax, Scrollbar, Swiper, Thumbs, Virtual, Zoom
  ...
  function loadInSlide(index, loadInDuplicate = true) {
    ...
    $images.each(imageEl => {
      ...
      swiper.loadImage($imageEl[0], src || background, srcset, sizes, false, () => {
        ...
        if (background) {
          $imageEl.css('background-image', `url("%24%7bbackground%7d.html")`);
...
function History({
  ...
  const getPathValues = urlOverride => {
    ...
    const pathArray = location.pathname.slice(1).split('https://metalslugawk.vnggames.com/').filter(part => part !== '');
  ...
  const setHistory = (key, index) => {
    ...
    if (swiper.params.history.root.length > 0) {
      ...
      if (root[root.length - 1] === 'https://metalslugawk.vnggames.com/') root = root.slice(0, root.length - 1);
...
// EXTERNAL MODULE: ./assets/main/js/scaleRoot.2.js
var scaleRoot_2 = __webpack_require__(778);
// EXTERNAL MODULE: ./node_modules/twbs-pagination/jquery.twbsPagination.js
var jquery_twbsPagination = __webpack_require__(965);
var jquery_twbsPagination_default = /*#__PURE__*/__webpack_require__.n(jquery_twbsPagination);
;// CONCATENATED MODULE: ./assets/main/js/_vportal.js
/* provided dependency */ var $ = __webpack_require__(755);
...
$.fn.vPortal = function(options) {
    ...
    if (
        ...
        console.log((jquery_twbsPagination_default()));
...
var loadNews = function(
        ...
        // url = `http://dev.metalslug.vnggames.com/app_dev.php/ph/news/guide.1.html`;
        url = url.replaceAll('///', 'https://metalslugawk.vnggames.com/');
        url = url.replaceAll('//', 'https://metalslugawk.vnggames.com/');
        url = '//' + url;
...
// EXTERNAL MODULE: ./node_modules/scrollwatch/dist/ScrollWatch-2.0.1.min.js
var ScrollWatch_2_0_1_min = __webpack_require__(886);
var ScrollWatch_2_0_1_min_default = /*#__PURE__*/__webpack_require__.n(ScrollWatch_2_0_1_min);
;// CONCATENATED MODULE: ./assets/main/js/scrollwatch.js
/* provided dependency */ var scrollwatch_$ = __webpack_require__(755);


scrollwatch_$.fn.scrollWatch = function (options) {
	var defaults = {
		before: () => {},
		options: {
			watchOnce: false,
			...
			onElementInView: function () {
				scrollwatch_$(".breadcrumb").removeClass("fixed");
				scrollwatch_$(".nav").removeClass("fixed");
			},
			onElementOutOfView: function () {
				scrollwatch_$(".breadcrumb").addClass("fixed");
				scrollwatch_$(".nav").addClass("fixed");
			},
		},
		after: () => {},
	};
	var settings = scrollwatch_$.extend(defaults, options);
	settings.before();
	// console.log(settings.options);
	var sw = new (ScrollWatch_2_0_1_min_default())(settings.options);

	settings.after();
};

// EXTERNAL MODULE: ./node_modules/jquery-mousewheel/jquery.mousewheel.js
var jquery_mousewheel = __webpack_require__(450);
;// CONCATENATED MODULE: ./assets/main/js/scrollFrame.2.js
/* provided dependency */ var scrollFrame_2_$ = __webpack_require__(755);
...
const scrollFrame = {
	...
	func: {
		getTotalFrame: () => scrollFrame_2_$(scrollFrame.el.scrollFrame).length,
		getScaleRatio: () => {
			if (scrollFrame_2_$(scrollFrame.el.fullPageWrapper).length > 0) {
				return scrollFrame_2_$(scrollFrame.el.fullPageWrapper).attr("data-scale-ratio");
			} else {
				return 1;
			}
		},
		...
		getFrames: () => {
			scrollFrame_2_$(scrollFrame.el.scrollFrame).each(function () {
				// scrollFrame.el.frame.push("#"+$(this).attr("id"));
				scrollFrame.el.frame.push({
					el: "#" + scrollFrame_2_$(this).attr("id"),
					offsetTop: scrollFrame_2_$("#" + scrollFrame_2_$(this).attr("id")).offset().top,
				});
			});
		},
		...
		scrollTo: (frameId) => {
			...
			scrollFrame_2_$(scrollFrame.el.scrollContainer).animate(
				{
					scrollTop: scrollTop,
				},
		...
		activeNavBullet: (frameId) => {
			if (scrollFrame_2_$(scrollFrame.el.nav).length > 0) {
				scrollFrame_2_$(scrollFrame.el.nav).removeClass("active");
				scrollFrame_2_$(scrollFrame.el.nav)
					.parent()
					.parent()
					.find('[href="' + frameId + '"]')
					.addClass("active");
		...
		isOpeningPopup: () => {
			return scrollFrame_2_$("html").hasClass("popup-opened");
		},
	},
	handleMousewheel: () => {
		scrollFrame_2_$(window).on("mousewheel", function (e) {
			// e.preventDefault();
			// e.stopPropagation();
	...
	handleNav: () => {
		if (scrollFrame_2_$(scrollFrame.el.nav).length > 0) {
			scrollFrame_2_$(scrollFrame.el.nav).on("click", function (e) {
				e.preventDefault();
				let frameToScroll = scrollFrame_2_$(this).attr("href");
				for (let i = 0, len = scrollFrame.el.frame.length; i < len; i++) {
	...
	init: () => {
		...
		if (window.location.hash) {
			...
			// scrollFrame.data.currentFrame = 0;
			// scrollFrame.data.currentFrameId = scrollFrame.el.frame[0].el;
			// scrollFrame.func.scrollTo(scrollFrame.data.currentFrameId);
		}
	},
};

/* harmony default export */ const scrollFrame_2 = (scrollFrame);
...
// EXTERNAL MODULE: ./assets/main/js/customSelect.js
var customSelect = __webpack_require__(549);
;// CONCATENATED MODULE: ./assets/main/js/setup.js
/* provided dependency */ var setup_$ = __webpack_require__(755);

// Scale
window.addEventListener("load", function () {
	setup_$("#wrapper").scalePlatform();
	scrollFrame_2.init();

	setup_$(window).on("resize", function () {
		setup_$("#wrapper").scalePlatform();
		// scrollFrame.init();
	});
});
...
// fancybox
setup_$(".click-openpopup").on("click", function (e) {
	const popupId = setup_$(this).attr("data-href");
	setup_$.fancybox.open({
		src: "#popup-base",
		type: "inline",
		afterLoad: function () {
			setup_$("html").addClass("popup-opened");
			setup_$(popupId).addClass("active");
		},
		afterClose: function () {
			setup_$("html").removeClass("popup-opened");
			setup_$(popupId).removeClass("active");
		},
	});
});

;// CONCATENATED MODULE: ./assets/main/js/_swiper.js
/* provided dependency */ var _swiper_$ = __webpack_require__(755);
...
_swiper_$.fn.initSwiper = function (options) {
    var defaults = {
        isNested: false,
        isSync: false,
    ...
    var settings = _swiper_$.extend(defaults, options);
    ...
    if (_swiper_$(this).length == 0) {
        return;
    }

    let swiperSelector = "#" + _swiper_$(this).attr("id");
    ...
    if (settings.isRenderBullet) {
        var hostSwiper = _swiper_$(this).attr("id");
        addon_renderBullet = {
            renderBullet: function (index, className) {
                var addOnClass = "";
                var addOnText = index + 1;

                var swiperName = _swiper_$("#" + hostSwiper + " .swiper-wrapper")
                    .children(".swiper-slide")
                    .eq(index)
                    .attr("data-swiper-name");

                var swiperCode = _swiper_$("#" + hostSwiper + " .swiper-wrapper")
                    .children(".swiper-slide")
                    .eq(index)
    ...
    if (settings.isNested) {
        _swiper_$(this)
            .find(".swiper")
            .each(function () {
                if (_swiper_$(this).attr("data-swiper-isSync") === undefined) {
                    _swiper_$(this).initSwiper(settings.nestedOptions);
                }
            });
    }
    ...
    if (settings.isSync) {
        var hostSwiper = _swiper_$(this).attr("id");
        var syncWith = _swiper_$(this).attr("data-sync-with");
...
/* harmony default export */ const _swiper = ((/* unused pure expression or super */ null && (swiperControl)));
// export {swiperControl, swiperAnimation};

// EXTERNAL MODULE: ./node_modules/@fancyapps/fancybox/dist/jquery.fancybox.js
var jquery_fancybox = __webpack_require__(155);
...
;// CONCATENATED MODULE: ./assets/main/main.js
...
swiper_esm/* default */.ZP.use([swiper_esm/* Navigation */.W_, swiper_esm/* Pagination */.tl, swiper_esm/* EffectCoverflow */.lI]);

window.addEventListener("load", () => {
	const observer = lozad_min_default()(".lazyload", {
		loaded: function (el) {
	...
	// scroll frame setup
	const frameIds = scrollFrame_2.el.frame.map((f) => f.el);
	const navs = main_$(".scrollFrameControl");
	const pins = main_$(".scrollwatch-pin");
	pins.each(function (index, el) {
		main_$(el).scrollWatch({
			options: {
				watch: `${frameIds[index]} .scrollwatch-pin`,
				watchOnce: false,
				onElementInView: function () {
					if (scrollFrame_2.data.aniComplete === true) {
						const currentFrame = scrollFrame_2.data.currentFrame;
						navs.eq(currentFrame).removeClass("active");
						scrollFrame_2.data.currentFrame = index;
	...
	main_$(".news-bg").each(function () {
		var postsTag = "#" + main_$(this).attr("id");
...
(() => {
"use strict";
/* harmony import */ var swiper__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(665);
/* provided dependency */ var $ = __webpack_require__(755);
...
window.addEventListener(
	...
	() => {
		...
		const bannerSwiper = new swiper__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .ZP(".banner-swiper-container .swiper", {
			direction: "horizontal",
...
window.addEventListener("load", () => {
    ...
    const charThumbSwiper = new swiper__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .ZP("#character-pagination", {
    ...
    const characterSwiper = new swiper__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .ZP(`#character-swiper-container .swiper`, {
    ...
    const weaponSwiper = new swiper__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .ZP(`#weapon-swiper-container .swiper`, {
    ...
    const weaponPagination = new swiper__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .ZP("#weapon-pagination", {
        direction: "horizontal",
        preloadImages: false,
        lazy: true,
        initialSlide: 0,
    ...
    const vehicleSwiper = new swiper__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .ZP(`#vehicle-swiper-container .swiper`, {
    ...
    const vehiclePagination = new swiper__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .ZP(`#vehicle-pagination`, {
        direction: "horizontal",
        preloadImages: false,
        lazy: true,
        initialSlide: 0,
...
window.addEventListener(
	...
	() => {
		...
		const hotFeatureSwiper = new swiper__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .ZP("#hot-feature-swiper", {
			initialSlide: 1,
			direction: "horizontal",
			slidesPerView: "auto",
			loop: true,
...
/******/ })()
;
//# sourceMappingURL=metalslug.js.map