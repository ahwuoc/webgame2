...
class System
{
    ...
    public function check_user($user, $pass)
    {
        ...
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `users` WHERE `username`='" . $user . "' AND `password`='" . $pass . "' ");
        $rowcount = mysqli_num_rows($result);
        if ($rowcount > 0) {
            return true;
        } else {
    ...
    {
        $id = $_SESSION['id'];
        $result = mysqli_query($this->connect_db(), "SELECT * FROM `users` WHERE `id`='" . $id . "'");
    ...
    public function vongquay_gift($id, $item, $type = null)
    {

        $res = mysqli_fetch_assoc(mysqli_query($this->connect_db(), "SELECT * FROM `vongquay_gift` WHERE `id_vongquay`='" . $id . "'"));
        $prefix = 'item_';
