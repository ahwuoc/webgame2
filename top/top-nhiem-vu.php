...
$query = "SELECT name, data_task FROM player WHERE data_task IS NOT NULL";
...
foreach ($players as $row) {
    $data = json_decode($row['data_task'], true);
    $nv_id = isset($data[0]) ? (int)$data[0] : 0;
    $subnv = isset($data[1]) ? (int)$data[1] : 0;
    $progress = isset($data[2]) ? (int)$data[2] : 0;
    $last_time = isset($data[3]) ? (int)$data[3] : 0;
...
// Sắp xếp như thứ hạng trong game
usort($player_tasks, function ($a, $b) {
    return
        $b['nv_id'] <=> $a['nv_id'] ?:
        $b['subnv'] <=> $a['subnv'] ?:
...
$top_players = array_slice($player_tasks, 0, 10);

// Hiển thị avatar Top 1–3
echo '<div class="top-container-nv">';
if (isset($top_players[1])) {
    $name = $top_players[1]['name'];
    $progress = "{$top_players[1]['nv_id']}-{$top_players[1]['subnv']} ({$top_players[1]['progress']})";
    echo '<div class="top-box-nv top-nv2">
            <img src="../image/avatar17.png" alt="Top 2 NV">
            <div class="top-rank-label">TOP 2</div>
            <div>' . $name . '</div>
            <div>' . $progress . '</div>
          </div>';
}
if (isset($top_players[0])) {
    $name = $top_players[0]['name'];
    $progress = "{$top_players[0]['nv_id']}-{$top_players[0]['subnv']} ({$top_players[0]['progress']})";
    echo '<div class="top-box-nv top-nv1">
            <img src="../image/avatar13.png"class="avatar-img" alt="Top 1 NV">
...
if (isset($top_players[2])) {
    $name = $top_players[2]['name'];
    $progress = "{$top_players[2]['nv_id']}-{$top_players[2]['subnv']} ({$top_players[2]['progress']})";
    echo '<div class="top-box-nv top-nv3">
            <img src="../image/avatar16.png" alt="Top 3 NV">
...
<table class="table table-striped table-hover table-bordered table-responsive mt-3 text-center" id="nhiemvu1">
    ...
    for ($i = 3; $i < count($top_players); $i++) {
        ...
        $progress_str = "{$player['nv_id']}-{$player['subnv']} ({$player['progress']})";
        $gifts = getGiftsForTopNV($rank);
        echo "<tr>
                <td class='text-danger fw-bold'>{$rank}</td>
                <td class='text-danger fw-bold'>{$player['name']}</td>
                <td class='text-danger fw-bold'>{$progress_str}</td>
    ...
    if (empty($top_players)) {
        echo "<tr><td colspan='4'>Không có dữ liệu để hiển thị</td></tr>";
					...
					function getGiftsForTopNV($topRank) {
                        if ($topRank == 1) {
